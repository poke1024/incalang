// ===========================================================================
//	drv_fill_oval.h			   ©2002-2002 Bernhard Liebl. All rights reserved.
// ===========================================================================
...
#define hline( x0, x1, y )												\
	if( y >= clip_top && y < clip_bottom ) {							\
		long px0, px1;													\
		if( x0 < clip_left ) px0 = clip_left;							\
		else if( x0 >= clip_right ) px0 = clip_right - 1;				\
		else px0 = x0;													\
		if( x1 < clip_left ) px1 = clip_left;							\
		else if( x1 >= clip_right ) px1 = clip_right - 1;				\
		else px1 = x1;													\
		if( px0 <= px1 ) {												\
			u08* pixel_ptr = base_ptr + y * pitch +						\
				px0 * DRV_BYTES_PER_PIXEL;								\
			DRV_PUT_HLINE( pixel_ptr, ( px1 - px0 + 1 ), color );		\
		}																\
	}
